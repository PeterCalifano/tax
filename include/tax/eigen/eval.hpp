...
/**
 * @brief Evaluate scalar TTE at a displacement type accepted by `TaylorExpansionT::eval`.
 */
template < typename T, int N, int M, typename Dx >
[[nodiscard]] constexpr T eval( const TaylorExpansionT< T, N, M >& f,
...
[[nodiscard]] auto eval( const Eigen::Tensor< TaylorExpansionT< T, N, M >, Rank >& t,
                         const Dx& dx )
    requires( Rank >= 1 )
{
...
[[nodiscard]] Eigen::Matrix< T, Dim, 1 > eval(
    const Eigen::Matrix< TaylorExpansionT< T, N, 1 >, Dim, 1 >& f, T dx ) noexcept
{
