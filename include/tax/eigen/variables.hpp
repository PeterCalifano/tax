...
/// TaylorExpansionT type.
template < typename >
struct expansion_traits;

template < typename T, int N, int M >
struct expansion_traits< TaylorExpansionT< T, N, M > >
{
    using scalar_type = T;
...
/// @brief True if `TTE` is a `TaylorExpansionT<T, N, M>` specialization.
template < typename >
inline constexpr bool is_tte_v = false;

template < typename T, int N, int M >
inline constexpr bool is_tte_v< TaylorExpansionT< T, N, M > > = true;
 ...
 * @tparam TTE The TaylorExpansionT type (e.g., `TEn<2, 4>`). `M` must equal the input size.
 ...
 * @tparam TTE The TaylorExpansionT type (e.g., `TEn<2, 3>`). `M` must match the vector
 * size.
