...
inline std::array< double, 4 > toLocal( const Vec4& ic, const BoxT& box )
{
    ...
    for ( int j = 0; j < 4; ++j )
    {
        out[ static_cast< std::size_t >( j ) ] =
            box.halfWidth( j ) > 0.0
                ? ( ic( j ) - box.center( j ) ) / box.halfWidth( j )
                : 0.0;
    }
    return out;
}
...
std::pair< double, double > posErrors(
    const DAState&                  state_at_t,
    const BoxT&                     leaf_box,
    const std::vector< Vec4 >&      mc_ic,
    const std::vector< Vec4 >&      mc_truth_at_t,
    const std::vector< int >&       mc_leaf_id,
    int                             this_leaf_id )
{
    double sum_sq = 0.0;
    double max_e  = 0.0;
...
template < int P >
Cell runTaylor( const BoxT&                    ic_box,
                const std::vector< Vec4 >&     mc_ic,
                ...
                double                         tFinal )
{
    ...
    auto       sol    = tax::ode::propagate( Verner89{}, rhs(),
                                             x0, 0.0, tFinal, cfg );
...
Cell runAdsLike( std::string                    name,
                 Criterion                      crit,
                 const BoxT&                    ic_box,
                 const std::vector< Vec4 >&     mc_ic,
                 const std::vector< Vec4 >&     mc_truth_final,
                 ...
                 double                         tFinal )
{
    ...
    const auto t0 = std::chrono::high_resolution_clock::now();
    auto       tree = tax::ads::propagate< P >( Verner89{}, crit, rhs(),
                                                ic_box, icCenter(),
                                                0.0, tFinal, cfg );
    const auto t1 = std::chrono::high_resolution_clock::now();
    const double ms = std::chrono::duration< double, std::milli >( t1 - t0 ).count();
    ...
    for ( int li : tree.done() )
    {
        ...
        const auto [ leaf_max, leaf_rms ] = posErrors( leaf.payload, leaf.box,
                                                       mc_ic, mc_truth_final,
                                                       mc_leaf, li );
        ...
        (void)leaf_max; (void)leaf_rms;
    }
    const double rms = n_used > 0
        ? std::sqrt( sum_sq / static_cast< double >( n_used ) ) : 0.0;
    return { name, P, tol, static_cast< int >( tree.done().size() ),
             max_e, rms, ms };
}

// ---- Sweep for a given P -------------------------------------------------
template < int P >
void sweepP( const BoxT&                    ic_box,
             const std::vector< Vec4 >&     mc_ic,
             const std::vector< Vec4 >&     mc_truth_final,
             const tax::ode::IntegratorConfig< double >& cfg,
             double                         tFinal,
...
int main()
{
    ...
    std::cout << "[validation] propagating " << kNMc
              << " Monte-Carlo samples..." << std::flush;
    std::mt19937                             rng( 42 );
    ...
    for ( int i = 0; i < kNMc; ++i )
    {
        ...
        auto sol = tax::ode::propagate< /*Dense=*/true >(
            Taylor< 16 >{}, rhs(), ic, 0.0, tFinal, ref_cfg );
        for ( int s = 0; s < kNSnaps; ++s )
            mc_truth[ static_cast< std::size_t >( s ) ].push_back(
                sol( times[ static_cast< std::size_t >( s ) ] ) );
    }
    ...
    //   reference orbit (200 sample times)
    ...
    for ( std::size_t i = 0; i < cells.size(); ++i )
    {
        ...
        out << "    { \"method\": \"" << c.method << "\""
            << ", \"P\": "          << c.P
            << ", \"tol\": "        << ( std::isnan( c.tol ) ? std::string( "null" )
                                                              : std::to_string( c.tol ) )
            << ", \"n_leaves\": "   << c.n_leaves
            << ", \"max_pos_err\": " << c.max_pos_err
            << ", \"rms_pos_err\": " << c.rms_pos_err
    ...
    {
        std::cout << "  " << std::setw( 7 ) << c.method
                  << "  " << std::setw( 3 ) << c.P
                  << "  " << std::setw( 10 )
                  << ( std::isnan( c.tol ) ? std::string( "  -" ) :
                       ( std::ostringstream{} << std::scientific << std::setprecision( 0 ) << c.tol ).str() )
                  << "  " << std::setw( 5 ) << c.n_leaves
                  << "  " << std::setw( 11 ) << std::scientific << std::setprecision( 3 ) << c.max_pos_err
                  << "  " << std::setw( 11 ) << c.rms_pos_err
                  << "  " << std::fixed << std::setprecision( 1 ) << std::setw( 8 ) << c.elapsed_ms << " ms"
                  << '\n';
    }
    return 0;
}
