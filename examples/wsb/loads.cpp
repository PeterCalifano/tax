// =============================================================================
// examples/wsb/loads.cpp
//
// Per-snapshot LOADS run on the WSB IC box. Same structure as ads.cpp;
// only the criterion changes (NLI / Losacco-Fossà-Armellin).
//
// Run:    ./wsb_loads
// Writes: wsb_loads.json
// =============================================================================
...
int main()
{
    ...
    const tax::ads::NliCriterion criterion{ /*tol=*/0.3, /*maxDepth=*/5 };
    ...
    std::ofstream out( "wsb_loads.json" );
    ...
    const auto    ref_times = tax::ode::linspace( 0.0, tFinal_days / kTimeU_days, kNRef );
    out << "  \"reference_orbit\": {\n";
    ...
    std::cout << "[wsb_loads] running " << kNSnaps
              << " per-snapshot LOADS propagations..." << std::flush;
    ...
    const std::vector< std::pair< std::string, std::string > > rows{
        ...
        { "output",          "wsb_loads.json" }
    };
    printBanner( "WSB loads (NLI piecewise polynomial flow)",
