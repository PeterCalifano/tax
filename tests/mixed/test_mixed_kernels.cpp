...
// computing the same expression. The box is a subset of the order-Σorder simplex
// and monomial degrees add, so no out-of-box factor can contribute to an in-box
// output.
...
// Check every in-box slot matches the isotropic reference.
...
// Seed constant away from 0 (sqrt domain); linear term on coord 1.
