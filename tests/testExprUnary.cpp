...
TEST(NegAddTo, NegNode_AddTo) {
    ...
    DAd<4> r1 = three + (-(x + two));
    DAd<4> r2 = three - (x + two);
    ExpectCoeffsNear(r1, r2);
}
...
TEST(NegAddTo, MulOfNegs) {
    ...
    auto y = DAd<4>::variable<0>({0.0}); y[0] = 3.0;
    DAd<4> r1 = (-x) * (-y);
    DAd<4> r2 =   x  *   y;
    ExpectCoeffsNear(r1, r2);
}
...
TEST(ScalarExpr, AddScalar_MatchesManual) {
    ...
    auto x = DAd<4>::variable<0>({1.0});
    DAd<4> r = x + 2.0;
    DAd<4> expected{}; expected[0]=3.0; expected[1]=1.0;
    ExpectCoeffsNear(r, expected);
}

TEST(ScalarExpr, ScalarChain) {
    // (x * 2 + 3 - 1) at x0=0: constant = 2, coeff[1] = 2
    auto x = DAd<3>::variable<0>({0.0});
    DAd<3> r = x * 2.0 + 3.0 - 1.0;
    EXPECT_NEAR(r[0], 2.0, kTol);
...
TEST(ScalarDivL, LeafDivisor_Linear) {
    // 2/(1+x): c[k] = 2*(-1)^k
    DAd<4> one_plus_x{}; one_plus_x[0]=1.0; one_plus_x[1]=1.0;
    DAd<4> r = 2.0 / one_plus_x;
    EXPECT_NEAR(r[0],  2.0, kTol);
    EXPECT_NEAR(r[1], -2.0, kTol);
