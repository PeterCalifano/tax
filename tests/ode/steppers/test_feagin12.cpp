// tests/ode/testFeagin12Stepper.cpp
//
// Stepper-level correctness of Feagin12Stepper on three RHS:
//   - dx/dt = x       (analytic exp)
//   - harmonic        (cos t, -sin t)
//   - exp single-step under the I controller
// Tolerances are tightened to 1e-13 to reflect the order-12
// propagation accuracy.

#include <gtest/gtest.h>

#include <Eigen/Core>
#include <cmath>

#include <tax/ode.hpp>

using tax::ode::Feagin12Stepper;
using tax::ode::IntegratorConfig;
using tax::ode::controllers::I;
using tax::ode::controllers::PI;

TEST( OdeFeagin12Stepper, ExponentialOneStep )
{
    using State = Eigen::Matrix< double, 1, 1 >;
    Feagin12Stepper< State > stepper;

    State x0; x0( 0 ) = 1.0;
    const auto f = []( const auto& x, double ) { return x; };

    IntegratorConfig< double > cfg;
    cfg.abstol = cfg.reltol = 1e-12;

    auto r = stepper.step( f, x0, 0.0, 0.1, cfg );

    EXPECT_TRUE( r.accepted );
    EXPECT_NEAR( r.x_new( 0 ), std::exp( 0.1 ), 1e-13 );

    auto x_at_t0 = Feagin12Stepper< State >::eval_dense(
        r.dense, 0.0, r.h_used, 0.0 );
    auto x_at_t1 = Feagin12Stepper< State >::eval_dense(
        r.dense, 0.0, r.h_used, r.h_used );
    EXPECT_NEAR( x_at_t0( 0 ), x0( 0 ),       1e-14 );
    EXPECT_NEAR( x_at_t1( 0 ), r.x_new( 0 ),  1e-14 );
}

TEST( OdeFeagin12Stepper, HarmonicOneStep )
{
    using State = Eigen::Matrix< double, 2, 1 >;
    Feagin12Stepper< State > stepper;

    State x0; x0( 0 ) = 1.0; x0( 1 ) = 0.0;
    const auto f = []( const auto& x, double )
    {
        State out;
        out( 0 ) =  x( 1 );
        out( 1 ) = -x( 0 );
        return out;
    };

    IntegratorConfig< double > cfg;
    cfg.abstol = cfg.reltol = 1e-12;

    auto r = stepper.step( f, x0, 0.0, 0.05, cfg );

    EXPECT_TRUE( r.accepted );
    EXPECT_NEAR( r.x_new( 0 ),  std::cos( 0.05 ), 1e-13 );
    EXPECT_NEAR( r.x_new( 1 ), -std::sin( 0.05 ), 1e-13 );
}

TEST( OdeFeagin12Stepper, ControllerIVariant )
{
    using State = Eigen::Matrix< double, 1, 1 >;
    Feagin12Stepper< State, I< double > > stepper;

    State x0; x0( 0 ) = 1.0;
    const auto f = []( const auto& x, double ) { return x; };

    IntegratorConfig< double > cfg;
    auto r = stepper.step( f, x0, 0.0, 0.05, cfg );
    EXPECT_TRUE( r.accepted );
    EXPECT_NEAR( r.x_new( 0 ), std::exp( 0.05 ), 1e-13 );
}
