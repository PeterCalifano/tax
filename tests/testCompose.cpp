...
TEST(Compose, SumOfProducts_AllLeaves) {
    ...
    auto [x, y, z] = DAMd<2,3>::variables({0.0, 0.0, 0.0});
    DAMd<2,3> r = x*x + y*y + z*z;
...
TEST(Compose, NegatedProductInSum) {
    ...
    auto [x, y, z] = DAMd<2,3>::variables({1.0, 1.0, 1.0});
    DAMd<2,3> r = x*y - z*z;
...
TEST(Compose, NestedSquareAndCube) {
    ...
    auto x = DAd<6>::variable<0>({1.0});
    DAd<6> r1 = square(cube(x));
    DAd<6> r2 = x * x * x * x * x * x;
    ExpectCoeffsNear(r1, r2);
}

TEST(Compose, SqrtOfProduct) {
    // sqrt(x*y) at (4,9) = 6
    auto [x, y] = DAMd<2,2>::variables({4.0, 9.0});
    DAMd<2,2> r = sqrt(x * y);
