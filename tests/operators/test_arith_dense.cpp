...
// scalar / TE must exist (mirrors the other mixed-scalar operators) and equal
// s * reciprocal(TE).
TEST( Arith, ScalarDivTE )
{
    auto x = tax::TE< 5 >::variable( 2.0 );
    auto a = 6.0 / x;  // 6 / x
    auto b = 6.0 * tax::reciprocal( x );
    tax::test::ExpectCoeffsNear( a, b, 1e-12 );
    EXPECT_DOUBLE_EQ( a.value(), 3.0 );  // 6 / 2
}
...
TEST( Arith, CompoundAssignTE )
{
    ...
    a += y;
    auto a_ref = x + y;
    for ( std::size_t k = 0; k < a.nCoefficients; ++k ) EXPECT_DOUBLE_EQ( a[k], a_ref[k] );

    auto s = x;
    s -= y;
    auto s_ref = x - y;
    for ( std::size_t k = 0; k < s.nCoefficients; ++k ) EXPECT_DOUBLE_EQ( s[k], s_ref[k] );

    auto m = x;
    m *= y;
    auto m_ref = x * y;
    for ( std::size_t k = 0; k < m.nCoefficients; ++k ) EXPECT_DOUBLE_EQ( m[k], m_ref[k] );

    auto d = x;
    d /= y;
    auto d_ref = x / y;
    for ( std::size_t k = 0; k < d.nCoefficients; ++k ) EXPECT_DOUBLE_EQ( d[k], d_ref[k] );
}
...
TEST( Arith, CompoundAssignScalar )
{
    ...
    for ( std::size_t k = 0; k < a.nCoefficients; ++k ) EXPECT_DOUBLE_EQ( a[k], ref[k] );
}
