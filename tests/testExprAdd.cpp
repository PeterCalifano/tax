...
TEST(AddTo, LeafSum_MatchesManual) {
    ...
    auto y = DAd<5>::variable<0>({0.0}); y[0]=2.0; y[1]=3.0;
    DAd<5> r1 = x + y;
    DAd<5> r2 = y + x;
...
TEST(AddTo, MulLeaves_Plus_MulLeaves) {
    ...
    DAd<4> r = x * two + x * three;   // x*2 + x*3 = 5x = 5 + 5δ
...
TEST(AddTo, MulLeaves_Plus_MulLeaves_vs_Reference) {
    ...
    auto y = DAd<5>::variable<0>({0.0}); y[0]=3.0;
    DAd<5> ref = (x * x) + (y * y);
    DAd<5> r   =  x * x  +  y * y;
...
TEST(AddTo, Correctness_Bivariate) {
    auto [x, y] = DAMd<3,2>::variables({0.0, 0.0});
    DAMd<3,2> r = x*x + y*y;
...
TEST(SubTo, MulLeaves_Minus_MulLeaves) {
    auto x = DAd<4>::variable<0>({1.0});
    DAd<4> two{2.0}, one{1.0};
    DAd<4> r = x * two - x * one;   // x*(2-1) = x = 1+δ
...
TEST(SubTo, MixedAddSubChain) {
    ...
    DAd<4> one{1.0}, two{2.0}, three{3.0};
    DAd<4> r = x + one - two + three;   // x + 2
...
TEST(SubTo, SubtractSelf_IsZero) {
    ...
    DAd<5> r = x - x;
    for (std::size_t k = 0; k < DAd<5>::ncoef; ++k)
        EXPECT_NEAR(r[k], 0.0, kTol) << "k=" << k;
}

TEST(SubTo, Bivariate_Subtraction) {
    auto [x, y] = DAMd<2,2>::variables({1.0, 2.0});
    DAMd<2,2> r = x - y;   // (1-2) + δx - δy
