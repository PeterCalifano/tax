...
Unchanged from the pre-Stage-1 code, adapted to the new type names. The
shared helper does coefficient-wise equality with `tol = 1e-12` (proven from
the original suite):
...
`testEigenVectorCalc`, `testInvert`) follow the same shape, calling the
appropriate DACE method on the reference side and the corresponding `tax::`
free function or member on the tested side.
